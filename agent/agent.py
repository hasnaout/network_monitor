...
DEFAULT_SERVER_URL = "http://192.168.120.45:8000/api/monitoring/ping/"
INTERVAL = 30
...
def get_agent_base_dir():
    ...
    return os.path.join(os.path.dirname(__file__), "agent")


def get_config_file():
    return os.path.join(get_agent_base_dir(), "agent.config.json")


def get_log_file():
    return os.path.join(get_agent_base_dir(), "agent.log")


def log_message(message):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    try:
        with open(get_log_file(), "a", encoding="utf-8") as stream:
...
def load_config():
    try:
        with open(get_config_file(), "r", encoding="utf-8") as stream:
...
def send_heartbeat():
    ...
    try:
        ...
        log_message(f"Echec heartbeat vers {get_server_url()}: {exc!r}")
...
class NetworkAgentService(win32serviceutil.ServiceFramework):
    ...
    def SvcStop(self):
        log_message("Arret du service demande.")
        self.ReportServiceStatus(win32service.SERVICE_STOP_PENDING)
        win32event.SetEvent(self.stop_event)
        self.running = False

    def SvcDoRun(self):
        log_message("Demarrage du service.")
        ...
        try:
            ...
            log_message(f"Erreur fatale dans le service: {exc!r}")
            raise
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        try:
            ...
            log_message(f"Echec du bootstrap du service: {exc!r}")
            raise
    else:
